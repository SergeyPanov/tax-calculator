...
- **DAP-compliant tax calculation** — Computes DPFO (daň z příjmů fyzických osob) following Czech tax return rules:
  ...
  - Progressive rates: 15 % up to `ANNUAL_THRESHOLD_CZK` (required from environment or `.env`), 23 % above (DAP ř. 57)
...
- `ANNUAL_THRESHOLD_CZK` is mandatory. The backend reads it from process environment and also loads repository `.env`.
- Missing, blank, non-numeric, or non-positive values fail with `ValueError`.

Example `.env`:

```dotenv
ANNUAL_THRESHOLD_CZK=1676052
```
...
```bash
# Start both services with env values from `.env` and compose overrides
